...
**当前状态：原生分支已公开推送，第三轮云端验证已运行；归档和 57 项单元测试通过，6 项 UI 测试通过 5 项。尚未上传 TestFlight 或提交 App Store。**
...
第三轮 Account 流程先通过账号服务不可用、重试及暂不发起 Apple 授权的检查，随后在日记输入处触发 XCTest 的“未取得键盘焦点”错误，尚未执行后续购买绑定与重启持久化断言。同轮 Clarity 流程中相同输入框的输入、保存与重启验证通过。当前仅针对输入就绪补充了键盘及焦点工具栏等待、输入值断言，保留完整业务检查；这项测试修复尚待下一轮云端验证，不能宣称全部通过。
...
工作流生成的是未使用 Apple 分发签名的归档，不能直接安装或提交。代码检查和模拟器通过之后，还需要确定最终 bundle ID、Apple 团队、真实隐私/支持网址、线上服务配置、App Store Connect 商品及签名，才能生成和上传正式构建。

测试配置中的月/年价格不是 App Store Connect 已发布的商品。当前原生工程的隐私、支持及服务地址默认留空；只复制源代码不会自动启用真实付费或聊天服务。发布前应通过对应的 build settings 注入已核实地址，不能将服务私钥或 Apple 私钥写入源码。
