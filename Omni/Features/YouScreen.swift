        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    privacySection("Your journal", "Your name, focus, feelings, intentions and connection reflections are stored on this device with iOS file protection. They are excluded from the app's device backup. There is no journal account, cloud sync, advertising SDK, or third-party analytics in this version. Deleting the app can remove these records, so export anything you want to keep.")
                    privacySection("Reflection guidance", "The reflection tools use authored prompts and your own words on your device. They do not send journal entries or private messages to an AI provider.")
                    privacySection("Saved memory & conversations", "Memory is off until you choose to enable it. Plus lets you add facts or explicitly save your own words from a conversation. These records are protected on this device and excluded from device backup. Existing memories and history remain available to view, correct, delete and export after Plus ends. Local memory and each signed-in account use separate archives; signing in does not upload your journal or local memories.")
                    privacySection("Optional account & sync", "When the service is configured, Sign in with Apple connects an Omni account. Apple verifies your identity; Omni stores an opaque account identifier, protected session credentials and subscription status. Sync account memory asks separately before uploading saved account memories, conversation history and memory/history settings. Sync is manual, with choices for conflicting copies. Online chat consent stays separate on each iPhone. Sign-out leaves a protected account cache on this device, accessible in Omni when that account signs in again.")
                    privacySection("Optional online conversations", "Online conversations require a configured service, account access, Plus and separate consent. Your message, up to eight recent messages from the current conversation and up to six relevant memories are sent through Omni's service to OpenAI. Your journal is not included. Chat itself does not save another server copy; a later memory sync can upload saved history. OpenAI's API data retention policies still apply. No online chat is sent in an unconfigured build.")
                    ...
                    privacySection("Purchases & reminders", "Apple processes subscriptions and payment information. Omni reads Apple-verified subscription status to unlock Plus. Optional reminders are scheduled on your device, with generic text that doesn't reveal your journal.")
